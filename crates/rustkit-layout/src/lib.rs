...
/// Negative leading is NOT clamped (n57): a `line-height: 1` heading whose
/// face has a >1em content area seats above its line top, as in Blink.
...
impl DisplayList {
    ...
    fn render_text(&mut self, layout_box: &LayoutBox) {
        if let BoxType::Text(ref raw_text) = layout_box.box_type {
            ...
            // command carries: paint seats the baseline at round(y + ascent),
            // so this is the Blink seat minus the fractional ascent (see
            // blink_baseline_offset); the leading in it is SIGNED.
...
mod tests {
    ...
    fn test_baseline_seat_without_leading_is_the_rounded_ascent() {
        ...
        // Negative leading is signed and floored (n57): a 16px line under
        // an 18px content area seats at floor(-1) + 15 = 14, as in Chrome
        // (repro negative-leading.html row 4: ink rows 151-166 both engines).
        assert_eq!(blink_baseline_offset(16.0, 15.46875, 3.375), 14.0);
        // 40px system-ui heading at line-height 1: floor(-3.5) + 39 = 35.
        assert_eq!(blink_baseline_offset(40.0, 38.671875, 8.4375), 35.0);
    }
    ...
    fn test_negative_leading_seats_the_baseline_above_the_line_top() {
        ...
        // Whole-pixel seat: floor(signed half-leading) + rounded ascent, minus
        // the fractional ascent paint adds back (blink_baseline_offset).
        let expected_half = blink_baseline_offset(32.0, m.ascent, m.descent) - m.ascent;
        assert!(expected_half < 0.0, "Arial at line-height 1 has negative leading");
        assert!(
            (y - (top + expected_half)).abs() < 0.01,
            "y_cmd {y} should be top {top} + {expected_half}"
        );
    ...
    fn test_explicit_line_height_seats_on_the_primary_face() {
        ...
        let expected_half = blink_baseline_offset(20.0, arial.ascent, arial.descent) - arial.ascent;
        assert!((y - (top + expected_half)).abs() < 0.01, "y_cmd {y}, expected top + {expected_half}");
