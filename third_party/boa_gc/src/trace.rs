...
impl Tracer {
    ...
    /// Traces through all the queued nodes until the queue is empty, skipping
    /// nodes that are already marked and marking each one before tracing it.
    ///
    /// HIWAVE PATCH: backported from boa_gc 0.22. In 0.20 the weak-mark phase
    /// traced queued nodes without this check, which never terminates on a
    /// cycle reachable from an ephemeron value.
    ///
    /// # Safety
    ///
    ...
    pub(crate) unsafe fn trace_until_empty(&mut self) {
        while let Some(node) = self.queue.pop_front() {
            // SAFETY: the caller guarantees every queued pointer is valid.
