...
impl Collector {
    ...
    fn mark_heap(
        ...
        for eph in weaks {
            ...
            // HIWAVE PATCH (backport of boa_gc 0.22 `Tracer::trace_until_empty`):
            // skip marked nodes and mark before tracing, so a cycle reachable from
            // an ephemeron or weak-map value is traced once instead of forever.
