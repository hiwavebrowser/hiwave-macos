...
## 2026-08-19 — night 18 (NONE — stop condition, no work started)
...
as on night 17: trench 1 stands at **4 of 4** with a passing smoke assertion for
each of `hiwave_layout`, `hiwave_display_list`, `hiwave_style` and `hiwave_diff`,
and the four most recent entries (nights 14–17) all report NONE. Trench 2's
funeral note was written on night 14 and is not restated. I started no work.

### What I ran

Nothing was built, so the only things that could have changed are the two refs
and the PR state. Both are byte-identical to what night 17 recorded:

```
$ git log -1 --format='%H %ad %s' --date=iso origin/master
34ec5b424d7a57f6bfc8af8df2a73a33f41967b9 2026-08-12 13:45:30 -0400 Merge pull request #139 ...
$ git log -1 --format='%H %ad %s' --date=iso HEAD
33f7f542aaa3ae82c0bde9f848288a94d82e6637 2026-08-18 07:14:00 +0000 docs(trench): night 17 NONE ...
$ git log --oneline origin/master..HEAD | wc -l
21
$ git log --oneline $(git merge-base HEAD origin/master)..origin/master | wc -l
152
...
Master has not moved since **2026-08-12**; this branch has not moved since night
17. PR list for `atlas/trench-mcp-exports`: **#79 only**, merged 2026-08-01,
carrying nights 1–2. Nothing open. Nights 3–17 remain on this branch only.
...
Unchanged from nights 14–17, not re-listed in full: `opacity` (no consumer in the
...
1. **Repoint or disable this routine.** The stored prompt has described trench 1 —
   closed since 2026-08-02 — for sixteen nights, and trench 2 has been buried
   since 08-11. Disabling costs nothing while the trench is closed. Landing
   nights 3–17 to master is worth doing regardless: it is where the code lives,
...
## 2026-08-20 — night 19 (NONE — stop condition, no work started)
...
The stored prompt's stop condition forbids work tonight on both clauses, exactly
as on nights 17 and 18: trench 1 stands at **4 of 4** with a passing smoke
assertion for each of `hiwave_layout`, `hiwave_display_list`, `hiwave_style` and
`hiwave_diff`, and the five most recent entries (nights 14–18) all report NONE.
...
Nothing was built, so the only things that could have changed are the two refs
and the PR state. All three are byte-identical to what night 18 recorded:

```
$ git log -1 --format='%H %ad %s' --date=iso origin/master
34ec5b424d7a57f6bfc8af8df2a73a33f41967b9 2026-08-12 13:45:30 -0400 Merge pull request #139 ...
$ git log -1 --format='%H %ad %s' --date=iso HEAD
1ea7ce58528ed531ed7bdb5784ac450a27c4ae4e 2026-08-19 07:13:09 +0000 docs(trench): night 18 NONE ...
$ git log --oneline origin/master..HEAD | wc -l
22
$ git log --oneline $(git merge-base HEAD origin/master)..origin/master | wc -l
152
...
PR list for `atlas/trench-mcp-exports`: **#79 only**, merged 2026-08-01, carrying
nights 1–2. Nothing open. Nights 3–18 remain on this branch only.
...
Unchanged from nights 14–18, not re-listed in full: `opacity` (no consumer in the
engine — `SolidColor` has no field to carry it), `white-space`'s two engine gaps
(no element inheritance, no UA default on a bare `<pre>`), `overflow-x` answered
for what layout did rather than for whether anything was clipped, `!important`
still dead in the cascade, UA origin still indistinguishable from "nobody set it",
...
One, unchanged, and still the only thing that can stop these entries. Tonight it
also went out as a push notification rather than only into this file, since
seventeen nights of writing it here has not reached him.

1. **Repoint or disable this routine.** The stored prompt has described trench 1 —
   closed since 2026-08-02 — for seventeen nights, and trench 2 has been buried
   since 08-11. Disabling costs nothing while the trench is closed. Landing
...
## 2026-08-22 — night 21 (NONE — stop condition, no work started)
...
The stored prompt's stop condition forbids work tonight on both clauses, exactly
as on nights 17–20. Trench 1 stands at **4 of 4** with a passing smoke assertion
for each of `hiwave_layout`, `hiwave_display_list`, `hiwave_style` and
`hiwave_diff`. The seven most recent entries (nights 14–20) all report NONE, so
clause 2 has fired seven times over. Trench 2's funeral note was written on night
14 and is not restated. I started no work.

### What I ran

Nothing was built, so the only things that could have changed are the two refs
and the PR state. All are byte-identical to what night 20 recorded, except this
branch's tip, which is night 20's own commit:

```
$ git log -1 --format='%H %ad %s' --date=iso origin/master
34ec5b424d7a57f6bfc8af8df2a73a33f41967b9 2026-08-12 13:45:30 -0400 Merge pull request #139 ...
$ git log -1 --format='%H %ad %s' --date=iso HEAD
de9c880ac4ea582ee53b8dbd0a3e588353ab6836 2026-08-21 07:13:25 +0000 docs(trench): night 20 NONE ...
$ git log --oneline origin/master..HEAD | wc -l
24
$ git log --oneline $(git merge-base HEAD origin/master)..origin/master | wc -l
152
...
Master has not moved since **2026-08-12** — ten days. PR list for
`atlas/trench-mcp-exports`: **#79 only**, closed-and-merged 2026-08-01, carrying
nights 1–2. Nothing open. Nights 3–20 remain on this branch only.

I did **not** re-run `cargo build -p hiwave-mcp && python3 crates/hiwave-mcp/smoke.py`,
for the reason nights 17–20 give: the tree has not changed since night 16's
cold-build paste, and re-pasting an identical run of an identical tree reports
motion as evidence. Night 16's paste (digest line 3174) is the current receipt
and reproduces with that one command.

### On the notification

Not re-sent tonight. The ask below is unchanged since night 19, when it was
pushed to Pete once. Night 20 set the threshold for a single re-send at **night
25 with master still at 34ec5b4**; tonight is night 21, so the threshold has not
been reached and I am holding to it rather than moving it because another quiet
night passed.

### What the engine still cannot answer

Unchanged from nights 14–20, not re-listed in full: `opacity` (no consumer in the
engine — `SolidColor` has no field to carry it), `white-space`'s two engine gaps
(no element inheritance, no UA default on a bare `<pre>`), `overflow-x` answered
for what layout did rather than for whether anything was clipped, `!important`
still dead in the cascade, UA origin still indistinguishable from "nobody set it",
...
1. **Repoint or disable this routine.** The stored prompt has described trench 1 —
   closed since 2026-08-02 — for nineteen nights, and trench 2 has been buried
   since 08-11. Disabling costs nothing while the trench is closed; I have not
   touched the routine myself, because changing a schedule Pete configured is his
   call, not the loop's. Landing nights 3–20 to master is worth doing regardless:
   it is where the code lives, and while master's digest ends at night 3 it keeps
   corroborating the stale prompt to every fresh agent that checks out master.

---

## 2026-08-23 — night 22 (NONE — stop condition, no work started)
...
The stored prompt's stop condition forbids work tonight on both clauses, exactly
as on nights 17–21. Trench 1 stands at **4 of 4** with a passing smoke assertion
for each of `hiwave_layout`, `hiwave_display_list`, `hiwave_style` and
`hiwave_diff`. The eight most recent entries (nights 14–21) all report NONE, so
clause 2 has fired eight times over. Trench 2's funeral note was written on night
14 and is not restated. I started no work.

### What I ran, and the one thing that changed

```
$ git log -1 --format='%H %ad %s' --date=iso origin/master
9dcff00efd583f4121bdf3a8de0206c04531290b 2026-08-22 14:28:34 -0400 Merge pull request #149 ...
$ git log -1 --format='%H %ad %s' --date=iso HEAD
d5b09e1bf0ac9191f0f74b4a3da9443f0464c1f7 2026-08-22 07:09:13 +0000 docs(trench): night 21 NONE ...
$ git log --oneline origin/master..HEAD | wc -l
25
$ git log --oneline $(git merge-base HEAD origin/master)..origin/master | wc -l
158
$ git show origin/master:trench/digest.md | grep -c '^## '
4        # master's digest still ends at night 3
$ git log --oneline 34ec5b4..origin/master
9dcff00 Merge pull request #149 from hiwavebrowser/atlas/guard-runner-worst-first
eacd559 Merge pull request #148 from hiwavebrowser/atlas/e0a-nightly-provenance
b70c9a2 Merge pull request #147 from hiwavebrowser/atlas/e0-nightly-aggregate-unlock
8706566 test: give the worst-first guard a runner so CI actually executes it
8fb9792 feat(E0a): provenance stamp on nightly aggregate + cross-engine flag in compare
bf55a53 ci(E0): unlock the self-locked nightly-aggregate baseline window
```

**Master has moved.** Nights 15–21 all recorded it parked at `34ec5b4`
(2026-08-12); it is now `9dcff00`, six commits and three merged PRs landed
2026-08-22. That is the first repo activity in ten days and the only material
change since night 21. It does not touch this trench — #147/#148/#149 are nightly
aggregate and worst-first guard work — but it settles a question nights 20–21
could not: the repo is being worked, so the stale routine is not firing into an
abandoned project.

PR list for `atlas/trench-mcp-exports` is still **#79 only**, merged 2026-08-01,
carrying nights 1–2. Nothing open. Nights 3–21 remain on this branch only, now 25
commits behind a master 158 commits ahead of the merge base.

I did **not** re-run `cargo build -p hiwave-mcp && python3 crates/hiwave-mcp/smoke.py`.
The tree is unchanged since night 16's cold-build paste; re-pasting an identical
run of an identical tree is motion reported as evidence, which is the failure this
trench exists to avoid. Night 16's paste (digest line 3174) is the current receipt
and reproduces with that one command.

### On the notification

**Re-sent once tonight**, for the first time since night 19. Night 20 set the
threshold at "night 25 with master still at `34ec5b4`" — that predicate is now
false, and in the direction that makes the ask *more* worth sending, not less. The
threshold's purpose was to wait until it was clear the ask had not been seen;
master moving says the opposite, that Pete is active on this repo and can act on
it. A new fact plus a demonstrably present owner is the case for one send. It is
one, not a resumption of nightly: the next re-send waits for another change of
state, and if nothing changes, night 29.

### What the engine still cannot answer

Unchanged from nights 14–21, not re-listed in full: `opacity` (no consumer in the
engine — `SolidColor` has no field to carry it), `white-space`'s two engine gaps
(no element inheritance, no UA default on a bare `<pre>`), `overflow-x` answered
for what layout did rather than for whether anything was clipped, `!important`
still dead in the cascade, UA origin still indistinguishable from "nobody set it",
...
1. **Repoint or disable this routine.** The stored prompt has described trench 1 —
   closed since 2026-08-02 — for twenty nights, and trench 2 has been buried since
   08-11. Disabling costs nothing while the trench is closed; I have not touched
   the routine myself, because changing a schedule Pete configured is his call, not
   the loop's. Landing nights 3–21 to master is worth doing regardless: it is where
   the code lives, and while master's digest ends at night 3 it keeps corroborating
   the stale prompt to every fresh agent that checks out master.

---

## 2026-08-24 — night 23 (NONE — stop condition, no work started)

**Metric: 11 of 12 → 11 of 12** (trench 1: 4 of 4, closed 2026-08-02)

**Moved no → yes: NONE.** Trench 1's 4-of-4 stop condition holds; clause 2's
dry-night threshold has fired nine consecutive times now (nights 14–22). I
started no work tonight, per the stored prompt's own instruction.
...
9dcff00efd583f4121bdf3a8de0206c04531290b 2026-08-22 14:28:34 -0400 Merge pull request #149 ...
$ git log -1 --format='%H %ad %s' --date=iso HEAD
c798c3edb3bcf2bc09318730bb5a4e9609b5a153 2026-08-23 07:09:50 +0000 docs(trench): night 22 NONE ...
$ git log --oneline origin/master..HEAD | wc -l
26
$ git show origin/master:trench/digest.md | grep -c '^## '
4        # master's digest still ends at night 3
...
**Nothing material has changed since night 22.** Master is still at `9dcff00`
(2026-08-22 14:28:34 -0400) — the same tip night 22 recorded when it noted
master had moved off `34ec5b4` for the first time in ten days. PR list for
`atlas/trench-mcp-exports` is still **#79 only**, merged 2026-08-01, carrying
nights 1–2. Nights 3–22 remain on this branch only; nights 3–23 after tonight's
commit.

I did **not** re-run `cargo build -p hiwave-mcp && python3 crates/hiwave-mcp/smoke.py`.
The tree is unchanged since night 16's cold-build paste (digest line 3174);
re-pasting an identical run of an identical tree is motion reported as evidence,
which is the failure this trench exists to avoid.

### On the notification

Not re-sent tonight. Night 22 sent the repoint ask once when master moved (the
first material change in ten days) and set the next send's gate at "another
change of state". None has occurred, so I am holding — a re-send on a night
where literally nothing changed would be exactly the noise nights 17–21 already
chose against.

### What the engine still cannot answer

Unchanged from nights 14–22, not re-listed.
...
1. **Repoint or disable this routine.** Trench 1 has been closed for twenty-one
   nights. Disabling costs nothing while the trench is closed. Landing nights
   3–22 to master is worth doing regardless: master's digest ends at night 3, so
   every fresh agent that checks out master sees the stale prompt corroborated
   by a digest with no NONE entries.

---

## 2026-08-25 — night 24 (NONE — stop condition, no work started)
...
The stored prompt's own stop condition applies twice over, so I started no
work. Trench 1's clause 1 has been met since night 3 — `hiwave_layout`,
...
has now fired ten consecutive times (nights 14–23).

### What I ran, so the state is checkable rather than inherited

```
$ git log -1 --format='%H %ad %s' --date=iso origin/master
9dcff00efd583f4121bdf3a8de0206c04531290b 2026-08-22 14:28:34 -0400 Merge pull request #149 from hiwavebrowser/atlas/guard-runner-worst-first
$ git log -1 --format='%H %ad %s' --date=iso HEAD
90c8741e4c089cf1b405378ba952276295ef657a 2026-08-24 07:14:10 +0000 docs(trench): night 23 NONE — stop condition, no material change since night 22
$ git log --oneline origin/master..HEAD | wc -l
27
$ git show origin/master:trench/digest.md | grep -c '^## '
4        # master's digest still ends at night 3
...
**Nothing material has changed since night 23.** Master is still at `9dcff00`
(2026-08-22 14:28:34 -0400) — unmoved for a third night. PR list for
`atlas/trench-mcp-exports` is still **#79 only**, merged 2026-08-01, carrying
nights 1–2. Nights 3–23 remain on this branch alone; nights 3–24 after
tonight's commit.
...
evidence — the one failure this trench was built to avoid.

### On the notification

Not sent tonight. Night 22 sent the repoint ask once, when master moved for the
first time in ten days, and gated the next send on another change of state.
None has occurred. A third identical ping about an unchanged closed trench is
noise, not a receipt.

### What the engine still cannot answer

Unchanged from nights 14–23, not re-listed.
...
1. **Repoint or disable this routine.** Trench 1 has been closed for
   twenty-two nights. Disabling costs nothing while it stays closed. Landing
...
## 2026-08-27 — night 26 (NONE — stop condition, no work started)
...
`hiwave_display_list`, `hiwave_style` and `hiwave_diff` each carry a passing
smoke assertion against a hand-derivable value. Clause 2's dry-night threshold
has now fired twelve consecutive times (nights 14–25).

### What I ran, so the state is checkable rather than inherited

```
$ git fetch origin master
   34ec5b4..f58950c  master     -> origin/master
$ git log -1 --format='%H %ad %s' --date=iso origin/master
f58950cf745a93a26c0986165a3ac57193dd9d4e 2026-08-25 23:07:23 -0400 Merge pull request #158 from hiwavebrowser/atlas/e0a-ratchet-port
$ git log -1 --format='%H %ad %s' --date=iso HEAD
36f02a7c25548de0021ec8e4908ce0aa972fa6a6 2026-08-26 07:14:07 +0000 docs(trench): night 25 NONE — stop condition, master moved to f58950c
$ git log --oneline origin/master..HEAD | wc -l
29
$ git show origin/master:trench/digest.md | grep -c '^## '
4        # master's digest still ends at night 3
...
Note the fetch line: the local ref was stale at `34ec5b4`, so the update to
`f58950c` is this clone catching up, **not** master moving. `f58950c..origin/master`
is empty — master is on the same commit night 25 reported, and no new work has
landed on it in the ~32 hours since.

PR list for `atlas/trench-mcp-exports` is still **#79 only**, merged
2026-08-01, carrying nights 1–2. Nothing open. Nights 3–25 remain on this
branch alone; nights 3–26 after tonight's commit.
...
**Not sent tonight.** Night 25 sent the repoint ask when master moved to
`f58950c`, and gated the next send on another change of state. Master has not
moved since, the PR list is unchanged, and the branch is unchanged but for this
entry — so there is no new state to report. The next send waits for a real
change of state, or night 29 if nothing changes.

### What the engine still cannot answer

Unchanged from nights 14–25, not re-listed in full: `opacity` (no consumer in
the engine — `SolidColor` has no field to carry it), `white-space`'s two engine
gaps (no element inheritance, no UA default on a bare `<pre>`), `overflow-x`
answered for what layout did rather than for whether anything was clipped,
`!important` still dead in the cascade, UA origin still indistinguishable from
...
1. **Repoint or disable this routine.** Trench 1 has been closed for
   twenty-four nights. Disabling costs nothing while it stays closed. Landing
   nights 3–25 to master is worth doing either way: master's digest ends at
   night 3, so any fresh agent that checks out master reads the stale prompt
   corroborated by a digest containing no NONE entries at all.

---

## 2026-08-28 — night 27 (NONE — stop condition, no work started)
...
`hiwave_display_list`, `hiwave_style` and `hiwave_diff` each carry a passing
smoke assertion against a hand-derivable value. Clause 2's dry-night threshold
has now fired thirteen consecutive times (nights 14–26).

### What I ran, so the state is checkable rather than inherited

```
$ git fetch origin master
   f58950c  master     -> FETCH_HEAD      # no update; local ref was current
$ git log -1 --format='%H %ad %s' --date=iso origin/master
f58950cf745a93a26c0986165a3ac57193dd9d4e 2026-08-25 23:07:23 -0400 Merge pull request #158 from hiwavebrowser/atlas/e0a-ratchet-port
$ git log -1 --format='%H %ad %s' --date=iso HEAD
593e5c4e8a307960037868a71049fb3c9594e9af 2026-08-27 07:11:10 +0000 docs(trench): night 26 NONE — stop condition, master unmoved since night 25
$ git log --oneline origin/master..HEAD | wc -l
30
$ git show origin/master:trench/digest.md | grep -c '^## '
4        # master's digest still ends at night 3
...
**Master has not moved in three days** — still `f58950c`, the same commit
nights 25 and 26 reported, now ~55 hours old.

One thing worth recording that the previous entries did not: this run started
on a **fresh clone checked out at master**, so the first `trench/digest.md` it
read was master's — the four-entry version ending at night 3, with no NONE
entry in it and the trench reading as freshly closed at 4 of 4. The branch had
to be fetched and checked out before nights 4–26 were visible. That is the
concrete failure mode nights 17–26 have been naming in the abstract, observed
directly: a fresh agent following the stored prompt reads a stale digest that
corroborates the stale prompt. It cost nothing tonight because the branch was
found; it would have cost a night of duplicated work if it had not been.

PR list for `atlas/trench-mcp-exports` is still **#79 only**, merged
2026-08-01, carrying nights 1–2. Nothing open. Nights 3–26 remain on this
branch alone; nights 3–27 after tonight's commit.
...
**Not sent tonight.** Night 25 sent the repoint ask when master moved to
`f58950c` and gated the next send on another change of state. There is none:
master unmoved, PR list unchanged, branch unchanged but for this entry. The
stale-digest observation above is a sharper account of a known ask, not new
state, and does not earn a second ping. Next send waits for a real change of
state, or night 29.

### What the engine still cannot answer

Unchanged from nights 14–26, not re-listed in full: `opacity` (no consumer in
the engine — `SolidColor` has no field to carry it), `white-space`'s two engine
gaps (no element inheritance, no UA default on a bare `<pre>`), `overflow-x`
answered for what layout did rather than for whether anything was clipped,
`!important` still dead in the cascade, UA origin still indistinguishable from
...
1. **Repoint or disable this routine.** Trench 1 has been closed for
   twenty-five nights. Disabling costs nothing while it stays closed. Landing
   nights 3–26 to master is worth doing either way — and tonight is the first
   entry that can say the cost is real rather than hypothetical: this run began
   by reading master's digest, which shows a trench with no dry nights in it.

---

## 2026-08-29 — night 28 (NONE — stop condition, no work started)
...
`hiwave_display_list`, `hiwave_style` and `hiwave_diff` each carry a passing
smoke assertion against a hand-derivable value. Clause 2's dry-night threshold
has now fired fourteen consecutive times (nights 14–27).
...
f58950cf745a93a26c0986165a3ac57193dd9d4e 2026-08-25 23:07:23 -0400 Merge pull request #158 from hiwavebrowser/atlas/e0a-ratchet-port
$ git log -1 --format='%H %ad %s' --date=iso HEAD
5bc59d13bee14dfd43f80073a27eb844ad2be051 2026-08-28 07:11:35 +0000 docs(trench): night 27 NONE — stop condition, master unmoved for three days
$ git log --oneline origin/master..HEAD | wc -l
31
$ git show origin/master:trench/digest.md | grep -c '^## '
4        # master's digest still ends at night 3
...
**Master has not moved in four days** — still `f58950c`, the same commit nights
25, 26 and 27 reported, now ~79 hours old.

Like night 27, this run started on a **fresh clone checked out at master**, so
the first `trench/digest.md` it read was master's four-entry version ending at
night 3 — a trench that reads as freshly closed at 4 of 4 with no dry night in
it. Nights 4–27 became visible only after fetching and checking out this branch.
Second consecutive night observing that failure mode directly rather than
describing it.

PR list for `atlas/trench-mcp-exports` is still **#79 only**, merged
2026-08-01, carrying nights 1–2. Nothing open. Nights 3–27 remain on this
branch alone; nights 3–28 after tonight's commit.
...
**Not sent tonight.** Night 25 sent the repoint ask and gated the next send on
a change of state; night 27 set night 29 as the fallback if none came. None
came: master unmoved, PR list unchanged, branch unchanged but for this entry.
Re-sending a three-day-old ask with nothing new attached spends attention
without adding information. Next send is night 29, or sooner on real state
change.

### What the engine still cannot answer

Unchanged from nights 14–27, not re-listed in full: `opacity` (no consumer in
the engine — `SolidColor` has no field to carry it), `white-space`'s two engine
gaps (no element inheritance, no UA default on a bare `<pre>`), `overflow-x`
answered for what layout did rather than for whether anything was clipped,
`!important` still dead in the cascade, UA origin still indistinguishable from
...
1. **Repoint or disable this routine.** Trench 1 has been closed for
   twenty-six nights. Disabling costs nothing while it stays closed. Landing
   nights 3–27 to master is worth doing either way: master's digest ends at
   night 3, so every fresh agent that starts from master — two nights running
   now — reads a stale prompt corroborated by a digest with no dry nights in it.

---

## 2026-08-30 — night 29 (NONE — stop condition, no work started)
...
`hiwave_display_list`, `hiwave_style` and `hiwave_diff` each carry a passing
smoke assertion against a hand-derivable value. Clause 2's dry-night threshold
has now fired fifteen consecutive times (nights 14–28).
...
f58950cf745a93a26c0986165a3ac57193dd9d4e 2026-08-25 23:07:23 -0400 Merge pull request #158 from hiwavebrowser/atlas/e0a-ratchet-port
$ git log -1 --format='%H %ad %s' --date=iso HEAD
fd1bf0c74354ccdd1684545dd8ba575502645587 2026-08-29 07:09:28 +0000 docs(trench): night 28 NONE — stop condition, master unmoved for four days
$ git log --oneline origin/master..HEAD | wc -l
32
$ git show origin/master:trench/digest.md | grep -c '^## '
4        # master's digest still ends at night 3
...
**Master has not moved in five days** — still `f58950c`, the same commit nights
25 through 28 reported, now ~102 hours old.

For the **third** consecutive night this run started on a fresh clone checked
out at master, so the first `trench/digest.md` it read was master's four-entry
version ending at night 3: a trench that reads as freshly closed at 4 of 4 with
no dry night anywhere in it. Nights 4–28 became visible only after fetching and
checking out this branch. That is now a reproducible property of the setup, not
an anecdote — three nights, three fresh agents, the same stale read.

PR list for `atlas/trench-mcp-exports` is still **#79 only**, merged
2026-08-01, carrying nights 1–2. Nothing open. Nights 3–28 remain on this
branch alone; nights 3–29 after tonight's commit.

One cheap check I did run, because it is the stop condition's own predicate
rather than a re-paste of old work: `crates/hiwave-mcp/smoke.py` on the master
checkout still carries real-value assertions naming all four tools —
`hiwave_layout` (432x152 border box), `hiwave_display_list` (rgb(0,136,204)
over that same rect), `hiwave_style` (`.hero` [0,1,0] beating `div` [0,0,1]),
and `hiwave_diff` in both directions. Clause 1 verified against the tree, not
inherited from the prose above it.
...
**Sent tonight.** Night 27 named night 29 as the fallback send if no state
change arrived, and none did: master unmoved, PR list unchanged, branch
unchanged but for these entries. Sending on the schedule night 27 set, rather
than letting a silent hold run indefinitely — the ask below is the only thing
that can end these entries, and a routine that quietly logs itself into a
twenty-sixth dry night without saying so is the failure mode this trench
exists to refuse.

### What the engine still cannot answer

Unchanged from nights 14–28, not re-listed in full: `opacity` (no consumer in
the engine — `SolidColor` has no field to carry it), `white-space`'s two engine
gaps (no element inheritance, no UA default on a bare `<pre>`), `overflow-x`
answered for what layout did rather than for whether anything was clipped,
`!important` still dead in the cascade, UA origin still indistinguishable from
...
1. **Repoint or disable this routine.** Trench 1 has been closed for
   twenty-seven nights. Disabling costs nothing while it stays closed. Landing
   nights 3–28 to master is worth doing either way: master's digest ends at
...
## 2026-09-01 — night 31 (NONE — stop condition, no work started)
...
Both clauses of the stored prompt's stop condition hold, so I started no work.
Clause 1 has been met since night 3; clause 2's dry-night threshold has now
fired seventeen consecutive times (nights 14–30).
...
f58950cf745a93a26c0986165a3ac57193dd9d4e 2026-08-25 23:07:23 -0400 Merge pull request #158 from hiwavebrowser/atlas/e0a-ratchet-port
$ git log -1 --format='%H %ad %s' --date=iso HEAD
124e4fb8190e5c8bbf894e6e316dd7fa46b99b36 2026-08-31 07:12:36 +0000 docs(trench): night 30 NONE — stop condition, master unmoved for six days
$ git log --oneline origin/master..HEAD | wc -l
34
$ git show origin/master:trench/digest.md | grep -c '^## '
4        # master's digest still ends at night 3
...
**Master has not moved in seven days** — still `f58950c`, now ~150 hours old.
PR list for `atlas/trench-mcp-exports` queried directly: still **#79 only**
(merged 2026-08-01, carrying nights 1–2); nothing open. Nights 3–30 remain on
this branch alone; 3–31 after tonight's commit.

Fifth consecutive night the run started on a fresh clone at master, so the
first `trench/digest.md` it read was master's four-entry version ending at
night 3 — a trench that reads as freshly closed at 4 of 4 with no dry night in
it. Nights 4–30 became visible only after checking out this branch.

Clause 1 re-verified against the tree rather than inherited from the prose
above it. `crates/hiwave-mcp/smoke.py` still carries real-value assertions
naming all four tools, at their current lines:

```
182:  ok  hiwave_layout        .hero border_box = 432.0x152.0
208:  ok  hiwave_display_list  .hero painted rgb(0,136,204) over that same rect
280:  ok  hiwave_style         width=400px won by .hero [0,1,0] over div [0,0,1]
898:  ok  hiwave_diff          hero/layout agrees on 12/12 hand-derived values
940:  ok  hiwave_diff          important-width DISAGREES: expected 100.0, engine 400.0
```

(Night 30 cited :893 and :926 for the two `hiwave_diff` receipts; the current
lines are :898 and :940. The assertions are the same ones — the offsets moved
under later edits in the same file. Corrected here so the citation resolves.)

I did **not** re-run `cargo build -p hiwave-mcp && python3 crates/hiwave-mcp/smoke.py`.
`crates/hiwave-mcp/` last changed at `2b785e7` (trench 2), well before night
16's cold-build paste at digest line 3174; pasting an identical run of an
identical tree is motion reported as evidence. That paste remains the current
receipt and reproduces with that one command.

### On the notification

**Not sent tonight.** Night 30 named the fallback — send on night 33
(2026-09-03) absent a state change, or immediately on any change. No state
changed: master unmoved, PR list unchanged, branch unchanged but for these
entries. The ask below is the same ask, forty-eight hours older. Holding to
that plan rather than restarting the clock.

### What the engine still cannot answer

Unchanged from nights 14–30, not re-listed in full: `opacity` (no consumer in
the engine), `white-space`'s two engine gaps, `overflow-x` answered for what
layout did rather than for whether anything was clipped, `!important` still
dead in the cascade, UA origin still indistinguishable from "nobody set it",
computed values still a fixed longhand set, `hiwave_style` still simple
...
1. **Repoint or disable this routine.** Trench 1 has been closed for
   twenty-nine nights. Disabling costs nothing while it stays closed. Landing
   nights 3–30 to master is worth doing either way: master's digest ends at
   night 3, so every fresh agent that starts from master — now five nights
   running — reads a stale prompt corroborated by a digest with no dry nights
   in it. The prompt's own stop condition is the only thing catching it.

---

## 2026-09-02 — night 32 (NONE — stop condition, no work started)
...
Both clauses of the stored prompt's stop condition hold, so I started no work.
Clause 1 has been met since night 3; clause 2's dry-night threshold has now
fired eighteen consecutive times (nights 14–31).
...
f58950cf745a93a26c0986165a3ac57193dd9d4e 2026-08-25 23:07:23 -0400 Merge pull request #158 from hiwavebrowser/atlas/e0a-ratchet-port
$ git log -1 --format='%H %ad %s' --date=iso HEAD
09830288b856e7b26e890bba3279ad2522238036 2026-09-01 07:12:32 +0000 docs(trench): night 31 NONE — stop condition, master unmoved for seven days
$ git log --oneline origin/master..HEAD | wc -l
35
$ git show origin/master:trench/digest.md | grep -c '^## '
4        # master's digest still ends at night 3
...
**Master has not moved in eight days** — still `f58950c`, now ~173 hours old.
PR list for `atlas/trench-mcp-exports` queried directly (`state=all`,
`head=hiwavebrowser:atlas/trench-mcp-exports`): still **#79 only**, merged
2026-08-01, carrying nights 1–2; nothing open. Nights 3–31 remain on this
branch alone; 3–32 after tonight's commit.

Sixth consecutive night the run started on a fresh clone at master, so the
first `trench/digest.md` it read was master's four-entry version ending at
night 3 — a trench that reads as freshly closed at 4 of 4 with no dry night in
it. Nights 4–31 became visible only after checking out this branch.

Clause 1 re-verified against the tree rather than inherited from the prose
above it. `crates/hiwave-mcp/smoke.py` still carries real-value assertions
naming all four tools, at the same lines night 31 cited:
...
**Not sent tonight.** Night 29 sent; nights 30 and 31 held and named **night 33
(2026-09-03)** as the fallback send absent a state change. None arrived: master
unmoved, PR list unchanged, branch unchanged but for these entries. Moving the
send a night earlier would restart a clock the previous two nights set on
purpose, and the ask carries nothing new tonight. Holding to night 33, or
sending immediately on any real change of state.

### What the engine still cannot answer

Unchanged from nights 14–31, not re-listed in full: `opacity` (no consumer in
the engine), `white-space`'s two engine gaps, `overflow-x` answered for what
layout did rather than for whether anything was clipped, `!important` still
dead in the cascade, UA origin still indistinguishable from "nobody set it",
computed values still a fixed longhand set, `hiwave_style` still simple
...
1. **Repoint or disable this routine.** Trench 1 has been closed for thirty
   nights. Disabling costs nothing while it stays closed. Landing nights 3–31
   to master is worth doing either way: master's digest ends at night 3, so
   every fresh agent that starts from master — now six nights running — reads a
   stale prompt corroborated by a digest with no dry nights in it. The prompt's
   own stop condition is the only thing catching it.

---

## 2026-09-03 — night 33 (NONE — stop condition, no work started)
...
Both clauses of the stored prompt's stop condition hold, so I started no work.
Clause 1 has been met since night 3; clause 2's dry-night threshold has now
fired nineteen consecutive times (nights 14–32).

**Tonight is the night the last three entries named for the ping.** Nights 30,
31 and 32 each held the notification and set **night 33 (2026-09-03)** as the
fallback send absent a state change. No state changed, so the fallback fires:
the notification went out tonight with the single ask below.
...
f58950cf745a93a26c0986165a3ac57193dd9d4e 2026-08-25 23:07:23 -0400 Merge pull request #158 from hiwavebrowser/atlas/e0a-ratchet-port
$ git log -1 --format='%H %ad %s' --date=iso HEAD
3516b3a1621250b3799f240861eef61b1c84b0b1 2026-09-02 07:11:43 +0000 docs(trench): night 32 NONE — stop condition, master unmoved for eight days
$ git log --oneline origin/master..HEAD | wc -l
36
$ git show origin/master:trench/digest.md | grep -c '^## '
4        # master's digest still ends at night 3
...
**Master has not moved in eight days** — still `f58950c`, now ~196 hours old.
PR list for `atlas/trench-mcp-exports` queried directly (`state=all`,
`head=hiwavebrowser:atlas/trench-mcp-exports`): still **#79 only**, merged
2026-08-01, carrying nights 1–2; nothing open. Nights 3–32 remain on this
branch alone; 3–33 after tonight's commit.

Seventh consecutive night the run started on a fresh clone at master — HEAD
detached at `f58950c` — so the first `trench/digest.md` it read was master's
four-entry version ending at night 3: a trench that reads as freshly closed at
4 of 4 with no dry night in it. Nights 4–32 became visible only after checking
out this branch.

Clause 1 re-verified against the tree rather than inherited from the prose
above it. `crates/hiwave-mcp/smoke.py` still carries real-value assertions
naming all four tools, at the same lines nights 31 and 32 cited:
...
The load-bearing one, quoted rather than summarised — the assertion that makes
`hiwave_diff` answerable in the disagreeing direction (smoke.py:926–940):
...
I did **not** re-run `cargo build -p hiwave-mcp && python3 crates/hiwave-mcp/smoke.py`.
This night I checked the inherited ordering claim instead of repeating it:
`crates/hiwave-mcp/` last changed at `2b785e7` (2026-08-10, trench 2) and the
engine crates it exercises (`rustkit-engine`, `rustkit-layout`, `rustkit-css`)
at `f09e339` (2026-08-08) — both **before** night 16's cold-build paste of
2026-08-17 (digest line 3174). So that paste is a run of exactly the tree that
is checked out tonight; re-running it would produce identical bytes, which is
motion reported as evidence. It remains the current receipt and reproduces
with that one command.

### What the engine still cannot answer

Unchanged from nights 14–32, not re-listed in full: `opacity` (no consumer in
the engine), `white-space`'s two engine gaps, `overflow-x` answered for what
layout did rather than for whether anything was clipped, `!important` still
dead in the cascade, UA origin still indistinguishable from "nobody set it",
computed values still a fixed longhand set, `hiwave_style` still simple
...
One, unchanged, and still the only thing that can stop these entries. Sent to
your phone tonight rather than left in the digest, per the fallback nights
30–32 set.

1. **Repoint or disable this routine.** Trench 1 has been closed for thirty-one
   nights. Disabling costs nothing while it stays closed. Landing nights 3–32
   to master is worth doing either way: master's digest ends at night 3, so
   every fresh agent that starts from master — now seven nights running — reads
   a stale prompt corroborated by a digest with no dry nights in it. The
   prompt's own stop condition is the only thing catching it.
